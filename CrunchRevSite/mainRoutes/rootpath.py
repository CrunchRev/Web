...
def setup():
    ...
    if ".ROBLOSECURITY" in cookiez:
        ...
        if info:
            ...
            if is_banned:
                if (not request.path.lstrip('/') == "not-approved") and (not "staticContent" in request.path.lstrip('/')) and (not request.path.lstrip('/') == "tos") and (not request.path.lstrip('/') == "logout"):
...
def secfunc2():
    return {"data":["b1f7ffcc4f8d04593018af283b8cb60f", "c5157659313d1213afe032fab30823db"]}, 200, {'Content-Type': 'application/json'}
