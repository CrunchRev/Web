...
pcall(function() game:GetService("Players"):SetBuildUserPermissionsUrl("http://www.%baseUrl%//Game/BuildActionPermissionCheck.ashx?assetId=0&userId=%d&isSolo=true") end)
...
game:GetService("ScriptContext"):AddCoreScriptLocal("LoadingScript", screenGui)
...
function doVisit()
	...
	player.CharacterAppearance = "http://assetgame.%baseUrl%/Asset/CharacterFetch.ashx?userId=%playerId%&placeId=0"
	...
	pcall(function() player:SetAccountAge(1000) end)
	
	if not inStudio and false then
		message.Text = "Setting Ping"
		visit:SetPing("http://assetgame.%baseUrl%/Game/ClientPresence.ashx?version=old&PlaceID=0", 120)
...
if success then
	...
	if not inStudio then
		if false then
			game:HttpPost("https://data.%baseUrl%/Error/Lua.ashx", "Visit.lua: " .. err)
		end
	end
end
